...
| `applications` | Visa applications (CORE) | 40+ fields + `tracking_code` + `portal_last_accessed` + `custom_fields` JSONB + `assignee` TEXT |
...
### Assignee System
Applications have an `assignee` TEXT column (not FK — just a simple string). Hardcoded assignees: `"deniz"` (blue, D) and `"sarpkan"` (orange, S) defined as `ASSIGNEES` array in both `applications-client.tsx` and `application-detail-page.tsx`. Assignee column in the list table shows colored circle with initial + dropdown to assign/unassign. Filter buttons in toolbar: Tümü | Deniz | Sarpkan | Atanmamış. Assignee also shown in detail page header with dropdown. DB update: `{ assignee: "deniz" | "sarpkan" | null }`.

### Application Detail Page — Portal Data as Source of Truth
Clicking a customer name in the applications list navigates to `/applications/[id]`, a full-page detail view. ALL customer data is rendered from `custom_fields` JSONB (the portal submission), grouped by section and ordered by `sort_order` from `portal_field_assignments` and `portal_smart_field_assignments` tables — mirroring the portal form layout. Section headers use the same i18n keys as the portal (`portalApply` namespace). Old vizebis admin columns (`company_id`, `passport_no`, `pickup_date`, `gender`, etc.) are dead — never populated for portal apps. The only admin-editable fields are: visa status, appointment date/time, finance fields, and notes. The list page table only shows: ID, Assignee, Tracking Code, Source, Full Name, Country, Appointment Date, Phone, Payment Status, Visa Status, Actions.

**Compact layout:** `ReadOnlyRow` component uses fixed-width label column (`140px`/`200px` on sm+) with always-visible copy button (clipboard icon) next to each field value. Date fields (detected via `field_type === "date"` for regular fields, key name containing "date" for smart fields) are formatted as dd/mm/yyyy with a `(dd/mm/yyyy)` hint appended to the label.

**Group visual grouping:** Applications with `group_id !== null` get a `border-l-[3px] border-l-violet-400` left accent in the table via `getRowClassName`.

### Field Assignment Data Model (IMPORTANT)
Regular field assignments (`portal_field_assignments`) are stored with **`country=NULL, visa_type=NULL`** — they're global/shared. Smart field assignments (`portal_smart_field_assignments`) are stored with **specific country+visa_type** values. When fetching for the detail page, you MUST query both: global assignments (`.is("country", null).is("visa_type", null)`) for regular fields, then country-specific for smart fields. Merging both into `fieldAssignments` array ensures all fields get correct section/sort_order. **Sections:** `personal_details` (name, surname, maiden_name, gender, email, phone, date_of_birth, birth_place, civil_status, nationality), `passport` (id_number, passport_no, date_issue, date_expiry, passport_country), `address`, `fingerprint`, `travel`, `employment`. The old `birth_info` and `nationality_civil` sections were merged into `personal_details`.
...
Document generation is **manual only** — triggered by admin clicking "Generate" buttons in the Generated Documents tab. Auto-generation on portal submission was removed. Generates booking PDF (via Python sidecar + pikepdf with Segoe UI font subsetting) and letter of intent (via Gemini 2.5 Flash API). Hotel selection is country-aware: filters by application's country first, falls back to any active hotel if no match. Group submissions share a single randomly-picked hotel via `sharedHotelId` parameter. Status tracked in `generated_documents` table (generating → ready/error). Client polls via setTimeout after triggering `/api/generate-documents`. **Timeout detection:** Records stuck in "generating" status for >5 minutes are treated as errors in the UI (`generated-documents-tab.tsx` useMemo).
