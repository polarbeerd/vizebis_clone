    ...
    <div className="w-full px-2">
      ...
      <div className="hidden sm:block">
        <div className="relative flex items-center justify-between">
          ...
          <motion.div
            className="absolute top-5 left-5 h-1 rounded-full bg-[#FEBEBF]"
            initial={{ width: "0%" }}
            animate={{
            ...
            return (
              <div key={step.key} className="relative z-10 flex flex-col items-center">
                ...
                >
                  {isCompleted ? (
                    <motion.div
                      className="flex h-10 w-10 items-center justify-center rounded-full bg-[#FEBEBF] text-white shadow-lg"
                      initial={{ scale: 0.8 }}
                      animate={{ scale: 1 }}
                    >
                    ...
                    <div className="relative">
                      <motion.div
                        className="absolute inset-0 rounded-full bg-[#FEBEBF]/30"
                        animate={{ scale: [1, 1.5, 1], opacity: [0.5, 0, 0.5] }}
                        transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
                      />
                      <div className="relative flex h-10 w-10 items-center justify-center rounded-full bg-[#FEBEBF] text-white shadow-lg shadow-brand-500/30">
                        <motion.div
          ...
          return (
            <div key={step.key} className="flex items-start gap-3">
              <div className="flex flex-col items-center">
                ...
                >
                  {isCompleted ? (
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#FEBEBF] text-white">
                    ...
                    <div className="relative">
                      ...
                      />
                      <div className="relative flex h-8 w-8 items-center justify-center rounded-full bg-[#FEBEBF] text-white">
                        <motion.div
                ...
                {!isLast && (
                  <div className={`w-0.5 h-8 ${
                    isCompleted ? "bg-[#FEBEBF]" : "bg-slate-200 dark:bg-slate-700"
                  }`} />
                )}
              </div>
              <motion.span
          ...
          <div key={step.key} className="flex items-start gap-3">
            <div className="flex flex-col items-center">
              <motion.div
                ...
                animate={{ scale: 1 }}
                transition={{ delay: 0.2 + index * 0.1, type: "spring" }}
                className="flex h-8 w-8 items-center justify-center rounded-full bg-[#FEBEBF] text-white"
              >
                <Check className="h-4 w-4" strokeWidth={3} />
              </motion.div>
              <div className="w-0.5 h-8 bg-[#FEBEBF]" />
            </div>
            <motion.span
              initial={{ opacity: 0, x: -10 }}
              animate={{ opacity: 1, x: 0 }}
